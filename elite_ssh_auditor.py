...
class PriViSSHReport(FPDF):
    def header(self):
        ...
        self.cell(0, 10, "PriVi-Elite SSH Security Audit Report", ln=True)
        self.set_xy(10, 20)
        self.set_font("Helvetica", "", 10)
        self.set_text_color(180, 180, 180)
        self.cell(0, 8, f"PriViSecurity  |  Auditor: {AUTHOR}  |  Tool: {TOOL} v{VERSION}", ln=True)
        self.ln(18)
    ...
    def finding_row(self, severity: str, description: str):
        ...
        self.set_text_color(255, 255, 255)
        self.cell(22, 6, f"  {severity}", fill=True, ln=False)
        self.set_font("Helvetica", "", 8)
        self.set_fill_color(245, 245, 245)
        self.set_text_color(30, 30, 30)
        self.cell(0, 6, f"  {description}", fill=True, ln=True)
        self.ln(1)


def generate_pdf_report(target: str, port: int, banner_info: dict,
                        version_cves: list, algo_audit: dict,
                        config_probe: dict, risk: dict) -> str:
    ...
    pdf.cell(55, 7, "  Security Score:", ln=False)
    bar_x = pdf.get_x()
    bar_y = pdf.get_y()
    ...
    if banner_info.get("reachable"):
        ...
        pdf.cell(0, 7, f"  Target unreachable: {banner_info.get('error', 'Unknown error')}", ln=True)
    pdf.ln(4)
    ...
    if version_cves:
        ...
        pdf.cell(0, 7, "  No known CVEs matched for detected version.", ln=True)
    pdf.ln(4)
    ...
    if algo_audit.get("method") and algo_audit["method"] != "unavailable":
        ...
        pdf.cell(0, 6, f"  Detection method: {algo_audit['method']}", ln=True)
        ...
        if not any([algo_audit["weak_ciphers"], algo_audit["weak_macs"], algo_audit["weak_kex"]]):
            pdf.set_font("Helvetica", "", 9)
            pdf.cell(0, 7, "  No weak algorithms detected.", ln=True)

        if algo_audit["host_keys"]:
            pdf.ln(2)
            pdf.set_font("Helvetica", "B", 9)
            pdf.cell(0, 6, "  Supported Host Key Types:", ln=True)
            pdf.set_font("Helvetica", "", 9)
            for hk in algo_audit["host_keys"]:
                pdf.cell(0, 5, f"    • {hk}", ln=True)
    else:
        pdf.set_font("Helvetica", "I", 9)
        pdf.set_text_color(100, 100, 100)
        pdf.cell(0, 7, "  Algorithm audit unavailable — nmap/ssh-keyscan not found.", ln=True)
        pdf.set_text_color(0, 0, 0)
    pdf.ln(4)
    ...
    if not config_probe.get("sshv1_supported") and not config_probe.get("banner_leaks_distro"):
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(0, 7, "  No observable configuration issues detected.", ln=True)
    pdf.ln(4)
    ...
    if risk["deductions"]:
        ...
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(0, 7, "  No risk deductions — strong configuration.", ln=True)
    pdf.ln(4)
...
def run_audit(target: str, port: int):
    """
